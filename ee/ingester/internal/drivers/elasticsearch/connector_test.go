...
import (
	...
	"github.com/formancehq/stack/libs/go-libs/testing/platform/elastictesting"
...
func TestElasticSearchConnector(t *testing.T) {
	...
	for i := 0; i < numberOfEvents; i++ {
		...
		go func(i int) {
			defer wg.Done()
			itemsErrors, err := connector.Accept(ctx, ingester.NewLogWithModule(moduleName, ingester.Log{
				Shard:   "test",
				ID:      fmt.Sprint(i),
				Date:    time.Now(),
				Type:    "test",
