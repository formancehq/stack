...
func EventFromType(t EventType, attempt *Attempt, hook *Hook) (Event, error) {
	...
	case FlushWaitingAttemptType:
		if(attempt == nil) {return Event{}, errors.New(fmt.Sprintf("BuildEvent: Missing %s for %s type", "attempt", t))}
	...
	case NewHookType :
		if(hook == nil) {return Event{}, errors.New(fmt.Sprintf("BuildEvent: Missing %s for %s type", "hook", t))}
...
func TypeFromEvent(ev Event) EventType {
	...
	if(ev.Channel == AttemptChannel){
		...
		if(ev.Attribute == "nexttry"){
			if(ev.ID != "") {return FlushWaitingAttemptType}
			return FlushWaitingAttemptsType
		}
		if(ev.Attribute == "abort"){
			if(ev.ID != ""){return AbortWaitingAttemptType}
		}
	}

	if(ev.Channel == HookChannel){
		if(ev.Attribute == "id") {return NewHookType}
		if(ev.Attribute == "status"){return ChangeHookStatusType}
		if(ev.Attribute == "endpoint") {return ChangeHookEndpointType}
		if(ev.Attribute == "secret") {return ChangeHookSecretType}
		if(ev.Attribute == "retry") {return ChangeHookRetryType}
