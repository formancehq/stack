...
The `fctl` command line is not the only way to execute Numscript. Formance Stack also exposes an HTTP API that accepts Numscript. This makes it much easier to integrate Numscript into your existing codebase.
