...
This is a great way to handle a simple case where we need to allow overdraft on an account. But sometimes we have business rules in place about negative balance accounts, for example the account @player:leslieknope can only allow overdraft up to 5 COIN, because if a player goes into more than 5 COIN in debt, he will lose the game.
...
The user `annperkins` will receive 5 coins in total, even if leslieknope only has 3 coins available. This would put her account balance at a negative 2 cones. 