...
Formance Ledger allows you to also add metadata to accounts. This metadata can be used to construct much more sophisticated transactions, with variables that bind to specific metadata at execution time. For more details on that usage, please refer to the Numscript metadata [reference](/ledger/reference/numscript/metadata).