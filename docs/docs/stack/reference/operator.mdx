...
In order to streamline feature-complete deployments the Formance stack, we provide a Kubernetes operator. The operator source code is available on [Github](https://github.com/formancehq/stack/tree/main/components/operator).
...
In addition to the dependencies listed above, the Formance stack requires a Kubernetes cluster with the following minimum requirements:
