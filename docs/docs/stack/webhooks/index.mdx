...
```shell
fctl webhook create "https://example.com/webhook" "ledger.transaction.created"
```
...
curl -X POST \
  ...
  -d '{
    ...
    "eventTypes": ["ledger.transaction.created"]
}'
```
...
The Formance Stack will sign each webhook request with a signature header. This signature can be used to verify that the request was sent by the Formance Stack. For each configured webhook, a secret key will be generated. This secret key can be retrieved directly from the webhook information:
