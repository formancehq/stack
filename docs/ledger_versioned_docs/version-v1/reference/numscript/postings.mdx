...
In Numscript, a [posting](/ledger/v1/reference/transactions#postings) models the movement of an amount of an asset from one account to another. Postings are wrapped in a [transaction](/ledger/v1/reference/transactions#transactions-1) to ensure that they are applied atomically.
