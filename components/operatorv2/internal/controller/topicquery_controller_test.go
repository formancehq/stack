		...
		It("Should create a Topic", func() {
			...
			Eventually(func() error {
				return Get(internal.GetResourceName(
					internal.GetObjectName(stack.Name, topicQuery.Spec.Service)), t)
			}).Should(BeNil())
				...
				Context("Then first TopicQuery object", func() {
					...
					It("Should remove the service from the queries of the topic", func() {
						Eventually(func(g Gomega) []string {
							topic := &v1beta1.Topic{}
							g.Expect(Get(internal.GetResourceName(internal.GetObjectName(stack.Name, topicQuery.Spec.Service)), topic)).To(Succeed())
					...
					Context("Then removing the last TopicQuery", func() {
						...
						It("Should completely remove the Topic object", func() {
							Eventually(func(g Gomega) bool {
								return errors.IsNotFound(Get(internal.GetResourceName(internal.GetObjectName(stack.Name, topicQuery.Spec.Service)), &v1beta1.Topic{}))
							}).Should(BeTrue())
						})
					})
				})
