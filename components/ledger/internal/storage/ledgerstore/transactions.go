...
func (store *Store) buildTransactionQuery(p PITFilterWithVolumes, query *bun.SelectQuery) *bun.SelectQuery {
	...
	query = query.
		...
		Join("join moves m on transactions.id = m.transaction_id and m.ledger = ?", store.name).
	...
	if p.ExpandEffectiveVolumes {
		query = query.ColumnExpr("get_aggregated_effective_volumes_for_transaction(?, transactions) as post_commit_effective_volumes", store.name)
	}
	if p.ExpandVolumes {
		query = query.ColumnExpr("get_aggregated_volumes_for_transaction(?, transactions) as post_commit_volumes", store.name)
	}
	return query
}
