...
create function upsert_account(_address varchar, _metadata jsonb, _date timestamp)
    returns bool
    language plpgsql
as $$
declare
...
begin
    select upsert_account(posting->>'source', _account_metadata->(posting->>'source'), _insertion_date) into source_created;
    select upsert_account(posting->>'destination', _account_metadata->(posting->>'destination'), _insertion_date) into destination_created;
...
create function insert_transaction(data jsonb, _date timestamp without time zone, _account_metadata jsonb)
    returns void
    language plpgsql
as $$
declare
...
begin
    if new.type = 'NEW_TRANSACTION' then
        perform insert_transaction(new.data->'transaction', new.date, new.data->'accountMetadata');
    end if;
    if new.type = 'REVERTED_TRANSACTION' then
        perform insert_transaction(new.data->'transaction', new.date, '{}'::jsonb);
...
create function get_aggregated_effective_volumes_for_transaction(tx numeric) returns jsonb
    stable
    language sql
as
$$
...
create function get_aggregated_volumes_for_transaction(tx numeric) returns jsonb
    stable
    language sql
as
$$
