...
func (s *Store) selectDistinctTransactionMetadataHistories(date *time.Time) *bun.SelectQuery {
	ret := s.db.NewSelect().
		...
		ModelTableExpr(s.GetTableName("transactions_metadata")).
...
func (s *Store) selectTransactions(date *time.Time, expandVolumes, expandEffectiveVolumes bool, q query.Builder) *bun.SelectQuery {
	...
	ret = ret.
		ModelTableExpr(s.GetTableName("transactions")).
		Column(
			"seq",
			"ledger",
			"id",
	...
	if s.ledger.HasFeature(ledger.FeaturePostCommitVolumes, "SYNC") && expandVolumes {
		ret = ret.
			Join(
				...
				s.db.NewSelect().
					TableExpr(s.GetTableName("moves")).
...
func (s *Store) insertTransaction(ctx context.Context, tx *Transaction) error {
	_, err := tracing.TraceWithLatency(ctx, "InsertTransaction", func(ctx context.Context) (*Transaction, error) {
		_, err := s.db.NewInsert().
			Model(tx).
			ModelTableExpr(s.GetTableName("transactions")).
			Value("id", "nextval(?)", s.GetTableName(fmt.Sprintf(`"%s_transaction_id"`, s.ledger.Name))).
			Returning("*").
			Exec(ctx)
		if err != nil {
			return nil, postgres.ResolveError(err)
...
// todo: handle not existing transactions
func (s *Store) RevertTransaction(ctx context.Context, id int) (*ledger.Transaction, bool, error) {
	var reverted bool
	tx, err := tracing.TraceWithLatency(ctx, "RevertTransaction", func(ctx context.Context) (*ledger.Transaction, error) {
		ret := &Transaction{}
		now := time.Now()
...
func (s *Store) UpdateTransactionMetadata(ctx context.Context, transactionID int, m metadata.Metadata) (*ledger.Transaction, error) {
	return tracing.TraceWithLatency(ctx, "UpdateTransactionMetadata", func(ctx context.Context) (*ledger.Transaction, error) {
		tx := &Transaction{}
...
func (s *Store) DeleteTransactionMetadata(ctx context.Context, id int, key string) (*ledger.Transaction, error) {
	return tracing.TraceWithLatency(ctx, "DeleteTransactionMetadata", func(ctx context.Context) (*ledger.Transaction, error) {
		ret := &Transaction{}
