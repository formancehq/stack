...
func TestWalletsPatch(t *testing.T) {
	...
	testEnv = newTestEnv(
		...
		WithAddMetadataToAccount(func(ctx context.Context, ledger, account string, md map[string]any) error {
			require.Equal(t, testEnv.LedgerName(), ledger)
			require.Equal(t, testEnv.Chart().GetMainBalanceAccount(w.ID), account)
			compareJSON(t, map[string]any{
				...
				wallet.MetadataKeyWalletCustomData: map[string]any{
					"role": "admin",
					"foo":  "baz",
				},
			}, md)
