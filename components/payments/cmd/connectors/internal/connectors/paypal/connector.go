...
func newConnector(logger logging.Logger, cfg Config) *Connector {
	return &Connector{
		logger: logger.WithFields(map[string]any{"component": "connector"}),
		cfg:    cfg,
	}
}

func (c *Connector) SupportedCurrenciesAndDecimals() map[string]int {
	return map[string]int{} // TODO: fill with values from the API's doc
}
...
func (c *Connector) UpdateConfig(ctx task.ConnectorContext, config models.ConnectorConfigObject) error {
	...
	shouldRestartTask := c.cfg.PollingPeriod.Duration != cfg.PollingPeriod.Duration
	c.cfg = cfg
