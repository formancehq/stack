...
export function WriteTransactions(BASE_URL, iterationInTest) {
    const url = new URL(`${BASE_URL}/transactions`);
    url.searchParams.append('async', "true");

    group('Transactions (WRITE)', function () {
        const res = http.post(url.toString(), JSON.stringify(generateTransactions(iterationInTest)), {
    ...
    group('Transactions - Numscript (WRITE)', function () {
        const res = http.post(url.toString(), JSON.stringify(generateTransactionsNumscript(iterationInTest)), {
            headers: { 'Content-Type': 'application/json' },
        });
        writeTransactionsWaitingTime.add(res.timings.waiting);
